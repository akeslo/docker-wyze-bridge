...
- **Dead Code:** `webrtc_stream.py` and `stream_manager.py` are legacy components not used by the current `go2rtc`-based implementation. `stream_manager.py` had zero references anywhere (including tests) and was deleted 2026-07-10. `webrtc_stream.py` is unused in production but still imported by `app/wyzebridge/tests/test_turn_reconnect.py`, so it was kept.
