...
- **Dead Code:** `webrtc_stream.py` and `stream_manager.py` were legacy components not used by the current `go2rtc`-based implementation. `stream_manager.py` had zero references anywhere (including tests) and was deleted 2026-07-10. `webrtc_stream.py` was unused in production and originally kept only because `app/wyzebridge/tests/test_turn_reconnect.py` still imported it; both the module and its always-skipped test were deleted together (`46ff4ba`), so neither exists in the tree anymore.
